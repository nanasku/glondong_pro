...
enum ReportType { transaksi, harian, bulanan, tahunan, penjual }
...
class _LaporanPembelianPageState extends State<LaporanPembelianPage> {
  ...
  List<Map<String, dynamic>> _daftarPenjual = [];
  String? _selectedPenjualId;
  String? _selectedPenjualNama;
  ...
  // Method untuk load daftar penjual dari database lokal
  Future<void> _loadDaftarPenjual() async {
    try {
      final penjualList = await databaseService.getAllPenjual();
      setState(() {
        _daftarPenjual = List<Map<String, dynamic>>.from(penjualList);
      });
    } catch (e) {
      debugPrint('Error loading daftar penjual: $e');
      _daftarPenjual = [];
    }
  }

  // Method untuk mengambil data pembelian dari database lokal
    ...
    try {
      ...
      String query = '''
        SELECT 
          ...
          p.created_at,
          p.total,
          penj.nama as nama_penjual,
          penj.id as penjual_id
        FROM pembelian p
        LEFT JOIN penjual penj ON p.penjual_id = penj.id
        WHERE 1=1
      ''';
      ...
      if (_selected == ReportType.harian && _selectedDate != null) {
        ...
        whereArgs.add(startDate.toIso8601String());
        whereArgs.add(endDate.toIso8601String());
      } else if (_selected == ReportType.penjual &&
          _selectedPenjualId != null &&
          _selectedPenjualId!.isNotEmpty) {
        query += ' AND p.penjual_id = ?';
        whereArgs.add(int.tryParse(_selectedPenjualId!) ?? 0);
      } else if (_selected == ReportType.transaksi &&
          _selectedFaktur != null &&
          _selectedFaktur!.isNotEmpty) {
        query += ' AND p.faktur_pemb LIKE ?';
      ...
      final pembelianList = await db.rawQuery(query, whereArgs);

      // Ambil detail dan operasional untuk setiap pembelian
      final List<Map<String, dynamic>> completeData = [];

      for (var pembelian in pembelianList) {
        ...
        final details = await db.rawQuery(detailQuery, [
          pembelian['faktur_pemb'],
        ]);

        // Ambil operasional pembelian
        final operasionalQuery = '''
          SELECT * FROM pembelian_operasional 
          WHERE faktur_pemb = ?
        ''';
  ...
  // Utility: group pembelian_detail into report structure
  Map<String, dynamic> _aggregateByKayu(List<dynamic> items) {
    ...
    for (var transaksi in items) {
      ...
      for (var d in details) {
        ...
        final harga = _safeGetNum(dynamicDetail, 'jumlah_harga_beli');
  ...
  Future<File> _createPdfReport() async {
    ...
    if (_selected == ReportType.transaksi && _singleTransaksi != null) {
      ...
      final title = 'LAPORAN PEMBELIAN TIAP TRANSAKSI';
          ...
          build: (ctx) {
            ...
            widgets.add(
              pw.Column(
                ...
                children: [
                  pw.Text(
                    'No. Faktur : ${_safeGetString(t, 'faktur_pemb') ?? '-'}',
                  ),
                  pw.Text(
                    'Tanggal    : ${_formatDateTime(_safeGetString(t, 'created_at'))}',
                  ),
                  pw.Text(
                    'Penjual    : ${_safeGetString(t, 'nama_penjual') ?? '-'}',
                  ),
                ],
              ),
            );
            ...
            for (var detail in details) {
              ...
              totalHargaDariDetails += _safeGetNum(detail, 'jumlah_harga_beli');
            }
      ...
      // LAPORAN HARIAN/BULANAN/TAHUNAN/PENJUAL
      String title = 'LAPORAN PEMBELIAN';
      if (_selected == ReportType.harian) title = 'LAPORAN PEMBELIAN HARIAN';
      if (_selected == ReportType.bulanan) title = 'LAPORAN PEMBELIAN BULANAN';
      if (_selected == ReportType.tahunan) title = 'LAPORAN PEMBELIAN TAHUNAN';
      if (_selected == ReportType.penjual)
        title = 'LAPORAN PEMBELIAN BERDASARKAN PENJUAL';
        ...
        pw.MultiPage(
          build: (ctx) {
            ...
            if (_selected == ReportType.penjual) {
              widgets.add(
                pw.Text(
                  'Nama Penjual : ${_data.isNotEmpty ? _data[0]['nama_penjual'] ?? '-' : '-'}',
                  textAlign: pw.TextAlign.left,
                ),
              );
            }
            ...
            for (var tx in _data) {
              ...
              table.add([
                ...
                pw.Text(tx['faktur_pemb'] ?? '-', textAlign: pw.TextAlign.left),
    ...
    final file = File(
      '${dir.path}/laporan_pembelian${DateTime.now().millisecondsSinceEpoch}.pdf',
    );
  ...
  Future<File> _createExcelReport() async {
    ...
    final dir = await getTemporaryDirectory();
    final file = File(
      '${dir.path}/laporan_pembelian_${DateTime.now().millisecondsSinceEpoch}.xlsx',
    );
          ...
          children: [
            ...
            TextField(
              decoration: InputDecoration(
                hintText: 'Contoh: PB-06102025-0001',
      ...
      case ReportType.penjual:
        return Column(
          ...
          children: [
            ...
            Container(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: [
                  ...
                  ..._daftarPenjual.map<DropdownMenuItem<String>>((penjual) {
                    final idStr = penjual['id']?.toString() ?? '';
                    return DropdownMenuItem<String>(
                      value: idStr,
                      child: Text(penjual['nama']?.toString() ?? 'Unknown'),
                    );
                  }).toList(),
                ],
                onChanged: (String? value) {
                  setState(() {
                    ...
                    if (value != null) {
                      final selected = _daftarPenjual.firstWhere(
                        (p) => p['id']?.toString() == value,
                        orElse: () => <String, dynamic>{},
                      );
                      _selectedPenjualNama = selected['nama']?.toString();
                    } else {
                      _selectedPenjualNama = null;
                    }
                  });
                },
              ),
            ...
            SizedBox(height: 8),
            if (_daftarPenjual.isNotEmpty)
              Text(
                '${_daftarPenjual.length} penjual tersedia',
            ...
            if (_data.isNotEmpty)
              Container(
                ...
                child: Text(
                  '${_data.length} transaksi ditemukan untuk penjual ini',
            ...
            children: [
              Wrap(
                ...
                children: [
                  ...
                  ChoiceChip(
                    label: Text('Penjual'),
                    selected: _selected == ReportType.penjual,
                    onSelected: (_) =>
                        setState(() => _selected = ReportType.penjual),
                  ),
                ],
              ),
              SizedBox(height: 12),
              ...
              if (_loading)
                ...
                ListView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Card(
                      child: ListTile(
                        title: Text(tx['faktur_pemb'] ?? '-'),
                        subtitle: Text(
                          '${tx['nama_penjual'] ?? '-'} - ${tx['created_at'] ?? '-'}',
                        ),
                        trailing: IconButton(
                          icon: Icon(Icons.picture_as_pdf),
                          onPressed: () async {
                            if (_selected == ReportType.transaksi) {
                              _selectedFaktur = tx['faktur_pemb'];
                              await _fetchForReport();
                              final f = await _createPdfReport();
                              await Share.shareXFiles(
                                [XFile(f.path)],
                                text: 'Laporan Transaksi ${tx['faktur_pemb']}',
                              );
                            }
                          },
                        ),
